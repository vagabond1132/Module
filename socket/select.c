...
int main(int argc, char *argv[])
{
    ...
    while (1)
    {
        ...
        ret = select(maxfd + 1,  &readmask, NULL, NULL, 0);
        ...
        if ( FD_ISSET(socket_fd, &readmask)) //当前的socket_fd 在select的轮训中;
        {
            ...
            printf("Local Server fd not in readmask\n");


        for ( count = 0; count < connNum; count++)
        {

            if ( FD_ISSET(ListenSock[count], &readmask) )
            {
                ret = recv(ListenSock[count], buf, sizeof(buf), 0);

                send(ListenSock[count], buf, sizeof(buf), 0 );
